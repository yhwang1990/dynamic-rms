...
public class MinSizeRMS {
    ...
    public MinSizeRMS(int dim, int k, double eps, int data_size, int init_size, int sample_size) {
        initializeDataset(dim, k, eps, data_size, init_size, sample_size);

        this.dualTree = new DualTree();
        this.setCover = new SetCover();
    }

    private void initializeDataset(int dim, int k, double eps, int data_size, int init_size, int sample_size) {
        Data.DIM = dim;
        Data.K = k;
        Data.EPS = eps;
