...
public class RunRMS_n_AC {
	...
	private static void runRMS(String dataPath, String tuplePath, String timePath) throws IOException {
		...
		for (int data_size = 100_000; data_size <= 1_000_000; data_size += 100_000) {
			...
			RMSInst inst = new RMSInst(dim, k, r, pair.eps, data_size, init_size, calcM(pair.pow, dim), data,
					samples);
	...
	private static Pair getParams(int dim, int k, int r, int data_size, int init_size, double old_eps, int fix_pow,
		...
		while (eps > 0.01) {
			...
			RMSInst test_inst = new RMSInst(dim, k, r, eps, data_size, init_size, fix_m, data, samples);
