...
public class RunRMS_mixed {
    ...
    private static void runRMS(String dataset, int k, int r, int dim, int size) throws IOException {
        ...
        ArrayList<Integer> opr_idxs = new ArrayList<>(), opr_types = new ArrayList<>();
        readMixedWorkload(paths[0], opr_idxs, opr_types);

        int data_size = data.length;
        int init_size = Math.max(data_size * 3 / 4, data_size - 25000) ;

        int maxM = dim + (1 << pow) + 1;
        double[][] samples = readUtilFile(dim, maxM);

        List<TupleOpr> workLoad = new ArrayList<>();
        for (int i = 0; i < opr_idxs.size(); ++i) {
        	if (opr_types.get(i) == 0) {
        		workLoad.add(new TupleOpr(opr_idxs.get(i), 1));
        	} else {
        		workLoad.add(new TupleOpr(opr_idxs.get(i), -1));
        	}
        }
        ...
        wr_time.write("init_time=" + Math.round(inst.initTime) + " oprs=" + workLoad.size() + "\n");
    ...
    private static String[] parseFilePath(String dataset, int dim, int k, int r, int size) {
        ...
        switch (dataset) {
            case "BB":
                ...
                paths[1] = "./result/NBA_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                paths[2] = "./result/NBA_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                break;
            case "AQ":
                paths[0] = "./dataset/AirQuality/AirQuality.txt";
                paths[1] = "./result/AQ_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                paths[2] = "./result/AQ_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                break;
            case "CT":
                paths[0] = "./dataset/CovType/CovType.txt";
                paths[1] = "./result/CT_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                paths[2] = "./result/CT_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                break;
            case "Movie":
                paths[0] = "./dataset/MovieLens/MovieLens.txt";
                paths[1] = "./result/Movie_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                paths[2] = "./result/Movie_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                break;
            case "Indep":
                if (size == 100000) {
                    paths[0] = "./dataset/Indep/Indep_" + dim + "d.txt";
                    paths[1] = "./result/Indep_" + dim + "d_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                    paths[2] = "./result/Indep_" + dim + "d_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                } else {
                    paths[0] = "./dataset/Indep/Indep1M_" + size + "_" + dim + "d.txt";
                    paths[1] = "./result/Indep_" + size + "_" + dim + "d_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                    paths[2] = "./result/Indep_" + size + "_" + dim + "d_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                }
                break;
            case "AntiCor":
                if (size == 100000) {
                    paths[0] = "./dataset/AntiCorr/AntiCorr_" + dim + "d.txt";
                    paths[1] = "./result/AC_" + dim + "d_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                    paths[2] = "./result/AC_" + dim + "d_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                } else {
                    paths[0] = "./dataset/AntiCorr/AntiCorr1M_" + size + "_" + dim + "d.txt";
                    paths[1] = "./result/AC_" + size + "_" + dim + "d_tuples_fdrms_k" + k + "_r" + r + "_mixed.txt";
                    paths[2] = "./result/AC_" + size + "_" + dim + "d_time_fdrms_k" + k + "_r" + r + "_mixed.txt";
                }
                break;
            default:
    ...
    private static void readMixedWorkload(String filePath, ArrayList<Integer> idxs, ArrayList<Integer> types) throws IOException {
        String wlPath = filePath.substring(0, filePath.length() - 4).split("_")[0] + "_mixed.txt";
        ...
        while ((line = br.readLine()) != null) {
        	...
        	idxs.add(Integer.parseInt(tokens[0]));
        	types.add(Integer.parseInt(tokens[1]));
        }
        br.close();
    }
